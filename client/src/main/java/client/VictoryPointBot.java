...
/**
 * 
 * Classe qui représente le Bot VictoryPoint qui fera des action qui lui rapport
 * le plus de victoryPoint a chaque fois
 *
 */
public class VictoryPointBot implements Action {
	 ...
	 */
	public VictoryPointBot(Client client) {
	...
	public boolean newAction() {
		if ((checkVictoryPointFace() || checkCarte()) && checkSolaryClient()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Methode qui renvoit s'il y a des face a victory point dans la forge
	 * 
	 * @return s'il y a des face a victory point dans la forge
	 */
	private boolean checkVictoryPointFace() {
	 ...
	 * 
	 * @param forgeCategory, forge category ou l'on cherche a savoir si le joueur
	 *        peut acheter
	...
	public int findFace(ForgeCategory f) {
		if (f.getFaces().size() == 1)
			...
			for (Face face : f.getFaces()) {
				if (face.getRessourceGranted().containsKey(Ressource.victoryPoint)) {
